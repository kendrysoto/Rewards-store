...
const Buy = () => {
    ...
    {state.posts.map((pro) =>
                <div className="Category-container" key={pro._id}>
                     {pro.cost > buy.point ? "Te faltan " + (pro.cost - buy.point) : <button className="ProducList-button" onClick={() => submitHandler(pro._id)}> <img  className="product-img" src="https://i.ibb.co/JCghNXv/buy-white11.jpg" /> </button>}
                 {submitHandler? buy.message: "no"}
